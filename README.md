...
Camera imagery uses Wi-Fi while the Bluetooth connection remains available for HID and compressed audio fallback. The Mac companion applies the stacked filters with Core Image. On machines with the signed OBS 30+ Camera Extension already active, MiniMate publishes directly into its producer sink—OBS itself does not need to run—and the phone appears to hardened applications through **OBS Virtual Camera**. The bundled legacy CoreMediaIO DAL plug-in remains available for macOS versions that still permit it; macOS 14.1 and newer block legacy cameras by default.
